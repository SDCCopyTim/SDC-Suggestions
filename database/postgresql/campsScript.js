...
// COPY camp(id, name, property, state, responses, rating, img_url, map_url) FROM '/Users/alphinalong/seniorHR/SDC/tim-suggestions/database/postgresql/camps.csv' DELIMITER ',' CSV HEADER;


const writeRecords = fs.createWriteStream('camps.csv');
writeRecords.write('id,name,property,state,responses,rating,img_url,map_url\n', 'utf8');
...
function writeTenMillionRecords(writer, encoding, callback) {
  ...
  function write() {
    ...
    do {
      ...
      const img_url = `https://timcamp.s3-us-west-1.amazonaws.com/camp${campImageNum}.jpg`; // images camp 1 - numImages
      const map_url = campMap[state];

      const data = `${id},'${name}','${property}','${state}',${responses},${rating},'${img_url}','${map_url}'\n`;
